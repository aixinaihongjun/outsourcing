          ...
          <div class="qm-widget-content">
            <div class="default-form">
              ...
              <div class="form-group row">
                <label class="col-sm-2 col-form-label">招募(可多选)</label>
                <div class="col-sm-9">
                  <span v-for="item in project.engineerTypeIdList" :key="item.id">
                    {{ item.positionName }} &nbsp;&nbsp;
                  </span>
                </div>
              </div>
              ...
              <div class="form-group row">
                <div class="col-sm-2 col-form-label"></div>
                <div class="col-sm-9">
                  <button @click="submitForm" type="button" class="theme-btn btn-style-one mr-3">立即发布</button>
...
export default {
  ...
  data() {
    return {
      ...
      dialogMsgTitle: "成功",
      dialogMsg: "", //点击发布后弹出的信息
      centerDialogVisible: false, //el dialog组件控制是否显示
      engineerTypeValue: "", //记录选择的工程师类型的id
      projectTypeData: [], //从后端获取的任务类型的数据
      engineerTypeData: [], //从后端获取的工程师类型的数据
      isNeedJobType: false, //是否需要选择职位类型
      // fileList: [],
      workWay: [
        //工作方式
        {
  ...
  methods: {
      ...
      if (this.$route.query.projectId) {
        this.axios
          ...
          .then((res) => {
            let {
              projectName,
              projectTypeList: projectTypeIdList,
              projectDuration,
              projectBudget,
              projectShortIntro,
              projectIntroduction,
              projectFileList: fileList,
              projectPositionTypeList: engineerTypeIdList,
              workExplain,
              startTime,
              endTime,
              isCompanyPublish,
            ...
            this.workWay.forEach((item) => {
              if (item.id == res.workType) {
                this.project.workType = item;
              }
            });
          });
      }
    ...
    handleSuccess(file) {
      this.project.fileList.push(file);
    },
    handleRemove(file) {
      this.project.fileList.splice(
        this.project.fileList.findIndex((elem) => elem.uid === file.uid),
        1
      );
    },
    checkForm() {
    ...
    /**
     * @description 确认按钮点击后调用此函数来发送输入的信息
     * @author 范明鑫
     */
    submitForm() {
      this.openMessageBox({
        title: "提示",
        content:
          "您发布的需求需要审核1-3个工作日，如有特殊要求，可以联系本站管理员。",
        handleConfirm: () => {
          this.project.projectTypeIdList = this.project.projectTypeIdList.map(
            (item) => {
              return item.id;
            }
          );
          this.project.engineerTypeIdList = this.project.engineerTypeIdList.map(
            (item) => {
              return item.id;
            }
          );
          this.project.workType = this.project.workType.id;
          if (this.$route.query.projectId) {
            this.axios
              ...
              .then(() => {
                ...
                setTimeout(() => {
                  this.$router.push("/workbench/my-tasks");
                }, 2000);
              });
          } else {
            this.axios.post("/project/publishTask", this.project).then(() => {
              this.$message({
                type: "success",
                message: "发布任务成功!",
              });
  ...
  /**
   * @description 首先从后端获取到招募类型和任务类型的数据
   * @author 范明鑫
   */
  created() {
    this.getData();
